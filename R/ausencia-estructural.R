...
.preparar_determinante <- function(x, nombre, n) {
  ...
  if (length(asomo) > .max_niveles_determinante) return(NULL)
  ...
  if (min(n_por_nivel) < .min_filas_nivel_determinante) return(NULL)
  list(
...
.hallazgo_ausencia_determinada <- function(columna, determinante, ajuste,
  ...
  .nuevo_hallazgo(
    ...
    paste0(
      ...
      ajuste$n_usables, " filas, con ", ajuste$n_niveles, " valores distintos",
      ...
      if (length(ajuste$niveles_aplica) == 1L) " es " else " est\u00e1 en ",
      .texto_valores_nivel(ajuste$niveles_aplica), "."
    ),
    paste0(
    ...
    for (det in determinantes) {
      ...
      if (is.null(mejor) || ajuste$cumplimiento > mejor$cumplimiento ||
          (ajuste$cumplimiento == mejor$cumplimiento &&
             ajuste$n_niveles < mejor$n_niveles)) {
        mejor <- ajuste
