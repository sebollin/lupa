...
- Reports `casi_clave` when a column is almost unique and its collisions
  concentrate in a few values, which separates a key with duplicates
  from free text of high cardinality.
...
measures every ready metric in its unconfirmed proposal by default,
aggregates it immediately, and retains the small dashboard rather than
row-level measurement detail. Use `medir_propuesta = FALSE` to disable
...
The API has a few boundaries worth knowing. There is no factory quality
score:
