...
test_that("el umbral de comparar_perfiles se compara con tolerancia", {
  ...
  # El caso existe: sin tolerancia los cuatro NO coinciden.
  expect_false(all(crudos) || !any(crudos))

  con_tolerancia <- vapply(pares, function(p) {
    (p[[2L]] - p[[1L]]) >= umbral - tolerancia
  }, logical(1L))
  expect_true(all(con_tolerancia))

  # Y el paquete usa esa forma: el texto de la comparacion lleva la tolerancia.
  cuerpo <- paste(
    deparse(getFromNamespace("comparar_perfiles", "lupa")), collapse = "\n"
  )
  expect_true(grepl("tolerancia", cuerpo, fixed = TRUE))
})
