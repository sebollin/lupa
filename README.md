...
Auditable data profiling and quality measurement: every result carries its
scope and its evidence.
...
Printed results keep the same formatted table even when the session uses
LC_CTYPE = C, where printing used to abort: in that locale R escapes the
characters it cannot represent as <U+00F1>. This includes the list columns in
the propuesta_modelo returned by proponer_modelo(); comparing two identical
profiles also stays silent in that locale.
...
| Profile against a database | `perfilar_dbi()` — full-table SQL aggregates plus, by default, a 112-analytic-field profile from a declared sample; `bloque_muestra = "solo_agregados"` requests only aggregates | [Profiling a database](https://sebollin.github.io/lupa/articles/perfilar-una-base.html) |
...
reporting how many it left out — just as `NA`s do, and just as the rows that
...
so nothing is suggested: it is read, in a single query chosen by the driver. And
